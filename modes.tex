...
could use \emph{modes}|<|the \CONTEXT\ way of doing conditional processing.
\StopAbstract
...
\stopitemize
The names are self explanatory. \type{\enablemode} activates a mode,
...
example,  you can  activate \type{screen} and \type{solution} modes by
\starttyping
    \enablemode[screen,solution]
\stoptyping

Modes can also be activated by a command line switch \type|-|\type|-modes| to
\type{texexec} and \type{context}. For example, to activate \type{screen} and
\type{solution} modes, you can run \CONTEXT\ using
\starttyping 
